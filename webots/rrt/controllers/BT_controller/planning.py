...
class Planning(py_trees.behaviour.Behaviour):
    ...
    def setup(self):
        ...
        set_motor_positions(
            self.leftBackMotor, self.leftFrontMotor, 
            self.rightBackMotor, self.rightFrontMotor, 
            float('inf')
        )
    ...
    def initialise(self):
        ...
        # Restrict lidar angles (as in your existing code)
        ...
        # Read the occupancy grid from blackboard and display it
        self.map = self.blackboard.read('map')
        display_map(self.display, self.map)

        # Get current robot position in map coords
        xw, yw = self.gps.getValues()[0], self.gps.getValues()[1]
        px, py = world2map(xw, yw)
        self.start = (px, py)

        # Convert user-supplied goal (world coords) to map coords
        wx, wy = self.goal
        gx, gy = world2map(wx, wy)
        self.goal = (gx, gy)

        # Create the occupancy grid map and a SLAM object
        ...
        # ----------------------------------------------------------
        # Call the RRT-based planner to get the initial path
        # ----------------------------------------------------------
        initial_path = rrt_search(self.grid_map, self.start, self.goal)

        # Convert the path from map to world coordinates
        self.WP = [map2world(x, y) for (x, y) in initial_path]

    def update(self):
        # If there's an obstacle close or near the end of the path, reduce WP_distance
    ...
    def recalculate_path(self):
        # Recompute path in presence of new obstacles
        self.slam.set_ground_truth_map(
            OccupancyGridMap(
                map_data=self.map,
                x_dim=self.map.shape[0],
                y_dim=self.map.shape[1]
        ...
        # Keep final WP as the ultimate goal
        wx, wy = self.WP[-1]
        gx, gy = world2map(wx, wy)
        self.goal = (gx, gy)

        # Re-scan environment for obstacles
        self.slam.rescan(global_position=self.start)

        # ----------------------------------------------------------
        # Rerun RRT-based planner
        # ----------------------------------------------------------
        path = rrt_search(self.grid_map, self.start, self.goal)

        # Convert new path to world coordinates
    ...
    def follow_waypoints(self):
        if not self.finish:
            ...
            # Mark current position in the display
            px, py = world2map(xw, yw)
            self.display.setColor(0xFF0000)
            self.display.drawPixel(px, py)

            # Place a marker at the next waypoint
            self.marker.setSFVec3f([*self.WP[self.index], 0])

            # Compute distance & heading to next WP
            rho = np.sqrt(
                (xw - self.WP[self.index][0])**2 + 
                (yw - self.WP[self.index][1])**2
            )
            alpha = np.arctan2(
                self.WP[self.index][1] - yw, 
                self.WP[self.index][0] - xw
            ) - theta
            alpha = (alpha + np.pi) % (2 * np.pi) - np.pi

            # If we are close enough, move on
            if rho < self.WP_distance:
                self.index += 1
                if self.index == len(self.WP):
                    self.finish = True

            # Simple P-control for wheel speeds
    ...
    def new_obstacle(self):
        ...
        # Mark newly found obstacles in local area
