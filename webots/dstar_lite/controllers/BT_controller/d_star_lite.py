...
import numpy as np
from grid_utils import heuristic,Vertices
...
class DStarLite:
    def __init__(self,map: OccupancyGridMap,s_start: (int,int),s_goal: (int,int)):
        ...
        self.sensed_map = OccupancyGridMap(map_data=map,
                                        x_dim=map.x_dim,
                                        y_dim=map.y_dim,
    ...
    def compute_shortest_path(self):
        while self.U.top_key() < self.calculate_key(self.s_start) or self.rhs[self.s_start] > self.g[self.s_start]:
            ...
            if k_old < k_new:
                ...
                pred.append(u)
                for s in pred:
                    if self.rhs[s] == self.cost(s, u) + self.g_old:
                        if s != self.s_goal:
                            min_s = float('inf')
                            succ = self.sensed_map.succ(vertex=s)
                            for s_ in succ:
                                temp = self.cost(s, s_) + self.g[s_]
                                if min_s > temp:
                                    min_s = temp
                            self.rhs[s] = min_s
    ...
    def move_and_replan(self, robot_position: (int, int)):
        ...
        while self.s_start != self.s_goal:
            ...
            arg_min = None
            for s_ in succ:
                temp = self.cost(self.s_start, s_) + self.g[s_]
                if temp < min_s:
                    min_s = temp
                    arg_min = s_
            ...
            if self.new_edges_and_old_costs:
                ...
                vertices = self.new_edges_and_old_costs.vertices                
                for vertex in vertices:                    
                    ...
                    for u, c_old in succ_v.items():
                        ...
                        if c_old > c_new:                            
                            if u != self.s_goal:
                                self.rhs[u] = min(self.rhs[u], self.cost(u, v) + self.g[v])
                        elif self.rhs[u] == c_old + self.g[v]:                            
                            if u != self.s_goal:                                                                
                                min_s = float('inf')
                                succ_u = self.sensed_map.succ(vertex=u)
                                for s_ in succ_u:
                                    temp = self.cost(u, s_) + self.g[s_]
                                    if min_s > temp:                                        
                                        min_s = temp
                                self.rhs[u] = min_s
                            self.update_vertex(u)
