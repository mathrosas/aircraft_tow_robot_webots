...
class Planning(py_trees.behaviour.Behaviour):
    ...
    def initialise(self):
        ...
        # Restrict lidar angles (as done in your existing code)
        ...
        # ----------------------------------------------------------
        # Instead of DStarLite, call our custom A* search:
        # ----------------------------------------------------------
        initial_path = a_star_search(self.grid_map, self.start, self.goal)

        # Convert path from map to world coordinates
        self.WP = [map2world(x, y) for (x, y) in initial_path]

    def update(self):
        # If there's an obstacle close or we are near the end of path, reduce WP_distance
    ...
    def recalculate_path(self):
        ...
        # ----------------------------------------------------------
        # Re-run A* search after the new obstacles are added
        # ----------------------------------------------------------
        path = a_star_search(self.grid_map, self.start, self.goal)

        # Convert new path to world coords
    ...
    def follow_waypoints(self):
        if not self.finish:
            ...
            # Draw robot's position on the display
            px, py = world2map(xw, yw)
            self.display.setColor(0xFF0000)
            self.display.drawPixel(px, py)

            # Show a marker at the next waypoint
            self.marker.setSFVec3f([*self.WP[self.index], 0])

            # Basic go-to-waypoint logic
    ...
    def new_obstacle(self):
        ...
        # Mark newly found obstacles in the local area
